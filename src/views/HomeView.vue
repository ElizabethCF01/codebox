    ...
    <div class="h-full flex justify-center w-full">
      <div class="w-full grid grid-cols-2 gap-2 mt-20 mb-2">
        <div class="bg-dark-gray flex flex-col gap-2 p-2">
          <prism-editor
            class="my-editor h-1/3 border-zinc-800"
            v-model="htmlCode"
        ...
        <div class="border border-zinc-700 flex flex-col transition-all ease-in">
          <iframe
            class="ide w-full h-full bg-white transition-all ease-in"
            :srcdoc="output"
          ></iframe>
        </div>
      </div>
...
const setCode = () => {
  output.value = `<div class="full">
    ...
    .log{ transition: all 0.3s ease; font-family: sans-serif; height: 30vh ;background: #181818; color:gray; padding: 8px; border-radius: 5px; display: flex; flex-direction: column; }${cssCode.value} </style>
    <script>
    var old = console.log;
			(function () {
				var old = console.log;
